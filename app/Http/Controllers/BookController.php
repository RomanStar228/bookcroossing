...
class BookController extends Controller
{
    ...
    /**
     * ===============================
     * Сохранение книги (все поля, кроме обложки, обязательны)
     * ===============================
     */
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'genre_id'    => 'required|exists:genres,id',           // теперь обязателен
            'description' => 'required|string|max:1000',            // обязателен
            'location'    => 'required|string|max:500',
            'city_id'     => 'required|exists:cities,id',           // обязателен
            'year'        => 'required|integer|min:1800|max:' . date('Y'), // обязателен
            'condition'   => 'required|string|max:100',              // обязателен
            'cover_image' => 'nullable|image|mimes:jpeg,png,jpg,webp|max:5120', // единственное необязательное
            'status'      => 'required|in:Отдаю,Ищу',                // обязателен
        ]);
        ...
        $book->city_id     = $validated['city_id'];
