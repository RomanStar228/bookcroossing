            ...
            <div class="overflow-x-auto">
                <table class="w-full min-w-[700px]">
                    <thead class="bg-[#F8F7F4]">
                        <tr>
                            <th class="px-4 sm:px-6 py-4 text-left">Настоящее имя</th>
                            <th class="px-4 sm:px-6 py-4 text-left">Логин</th>
                            <th class="px-4 sm:px-6 py-4 text-left">Почта</th>
                    ...
                    <tbody class="divide-y divide-[#e3e3e0]">
                        ...
                        <tr>
                            <td class="px-4 sm:px-6 py-5">{{ $user->full_name }}</td>
                            <td class="px-4 sm:px-6 py-5 text-[#706f6c]">{{ $user->username }}</td>
                            <td class="px-4 sm:px-6 py-5">{{ $user->email }}</td>
                            <td class="px-4 sm:px-6 py-5">{{ $user->city?->name ?? '—' }}</td>
                                    ...
                                    <div class="flex flex-col sm:flex-row justify-center gap-2">
                                        <form method="POST" action="{{ route('admin.users.ban', $user) }}" class="inline">
                                            ...
                                            <button onclick="return confirm('Заблокировать пользователя?')" 
                                                    class="bg-gray-800 hover:bg-gray-500 text-white px-3 py-2 rounded-lg text-sm w-full sm:w-auto">
                                                Заблокировать
                                            </button>
                                        </form>
                                        ...
                                        <form method="POST" action="{{ route('admin.users.destroy', $user) }}" class="inline">
                                            ...
                                            <button onclick="return confirm('Удалить пользователя навсегда?')" 
                                                    class="bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-lg text-sm w-full sm:w-auto">
                                                Удалить
                                            </button>
                                        </form>
                                    </div>
        ...
        <div class="bg-white rounded-3xl shadow-sm border border-[#e3e3e0] overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full min-w-[600px]">
                    <thead class="bg-[#F8F7F4]">
                        <tr>
                            <th class="px-4 sm:px-6 py-4 text-left">Имя</th>
                            <th class="px-4 sm:px-6 py-4 text-left">Username</th>
                            <th class="px-4 sm:px-6 py-4 text-left">Email</th>
                            <th class="px-4 sm:px-6 py-4 text-center">Действия</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-[#e3e3e0]">
